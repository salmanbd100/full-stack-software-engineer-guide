...
- [Chapter ?? — Service Boundaries and the API Gateway](#ch-service-boundaries) — the edge layer that usually owns the coarse limits
- [Chapter ?? — GraphQL, tRPC and Typed API Choices](#ch-graphql) — why one request is not one unit of work, and how complexity scoring fixes it
