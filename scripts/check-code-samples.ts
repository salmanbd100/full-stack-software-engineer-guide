 ...
 *
 * ## React is typed for real. Everything else is not
 *
 * Improvement #89 installed `@types/react` and `@types/react-dom`, so the 120 `tsx` fences —
 * Part III is the largest part in the book — compile against React's real declarations. A
 * real resolution beats the wildcard below, so no ambient change was needed: TypeScript only
 * falls back to `declare module "*"` when nothing else resolves.
 *
 * For every other import — next, express, zod, vitest, playwright and a dozen more —
 * `ambient.d.ts` still resolves to `any`, deliberately. Pinning them all would mean re-pinning
 * them every time the book's version stamps move, and React's types are the exception worth
 * paying for because they are stable across minors in a way the others are not. The
 * consequence is honest and worth stating: **outside React, this script proves the samples
 * parse and are internally consistent; it does not prove they match any library's current
 * API.** That is what `check:versions` and the Context7 lookups in CLAUDE.md are for.
 */
...
//
// The fallback. Anything that does not resolve for real — next, express, zod, vitest and
// the rest — lands here as \`any\`. React does resolve for real (#89), and a real
// resolution wins over this wildcard. See the script's header.
declare module "*";
